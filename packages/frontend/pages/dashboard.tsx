...
export default function Dashboard() {
  ...
  const loadMetrics = useCallback(async () => {
    try {
      const result = await trpc.portfolio.metrics.query();
      setMetrics(result);
    } catch (error) {
  ...
  const loadPositions = useCallback(async (brokerType: BrokerType) => {
    try {
      const result = await trpc.portfolio.positions.query({ brokerType });
  ...
  const handlePlaceOrder = async () => {
    ...
    setPlacing(true);
    try {
      const result = await trpc.orders.place.mutate({
        brokerType: broker,
      ...
      alert(error?.message || 'Order placement failed. Is this broker connected?');
    } finally {
      setPlacing(false);
    }
  };
      ...
      <header className="bg-white shadow">
        <div className="max-w-7xl mx-auto px-4 py-6 flex justify-between items-center">
          ...
          <div className="space-x-4">
            <select
              value={broker || ''}
              onChange={(e) => setBroker((e.target.value || null) as BrokerType | null)}
            ...
            <button onClick={() => router.push('/connect')} className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">
              Connect Broker
            </button>
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-8">
          <div className="bg-white rounded-lg shadow p-4">
            ...
            <p className="text-2xl font-bold">{loading ? '...' : `\u20B9${metrics.totalValue.toLocaleString()}`}</p>
          </div>
          <div className="bg-white rounded-lg shadow p-4">
            ...
            <p className={`text-2xl font-bold ${metrics.totalPnL >= 0 ? 'text-green-600' : 'text-red-600'}`}>
              {loading ? '...' : `\u20B9${metrics.totalPnL.toLocaleString()}`}
            </p>
          </div>
          <div className="bg-white rounded-lg shadow p-4">
            <p className="text-gray-600 text-sm">Day P&L</p>
            <p className={`text-2xl font-bold ${metrics.dayPnL >= 0 ? 'text-green-600' : 'text-red-600'}`}>
              {loading ? '...' : `\u20B9${metrics.dayPnL.toLocaleString()}`}
            </p>
          </div>
          <div className="bg-white rounded-lg shadow p-4">
            <p className="text-gray-600 text-sm">Margin Used</p>
            <p className="text-2xl font-bold">{loading ? '...' : `\u20B9${metrics.marginUsed.toLocaleString()}`}</p>
          </div>
          <div className="bg-white rounded-lg shadow p-4">
            <p className="text-gray-600 text-sm">Buying Power</p>
            <p className="text-2xl font-bold text-blue-600">{loading ? '...' : `\u20B9${metrics.buyingPower.toLocaleString()}`}</p>
          </div>
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-lg shadow p-6">
              ...
              {!broker ? (
                <p className="text-gray-500">Select a broker to view positions</p>
              ) : positions.length === 0 ? (
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {positions.map((pos, i) => (
                      <tr key={`${pos.symbol}-${i}`} className="border-b">
                        <td className="py-2">{pos.symbol}</td>
                        <td className="text-right">{pos.qty}</td>
                        <td className="text-right">{'\u20B9'}{pos.entryPrice}</td>
                        <td className="text-right">{'\u20B9'}{pos.currentPrice}</td>
